...
除 Excel 批量导入外，支持直接输入一段自然语言描述的案件文本，AI 自动识别并提取结构化字段。任意格式的自由文本（语音转文字、聊天记录、工作简报等）均可输入，AI 提取当事人、纠纷类型、乡镇、金额、难度、摘要后以表格预览展示，人工确认一键入库。

- 三级降级：DeepSeek-v4-pro（置信度 0.9）→ Ollama 本地模型（0.7）→ 正则规则兜底（0.4）
- 与去重/报告共享同一套 `AI_BACKEND` 环境变量，无需额外配置
- 解析结果展示引擎名 + 置信度，全程可追溯
- 量化效果：从手动填写 18 个字段约 2 分钟 → 贴入文字 5 秒确认
...
- **Prometheus 监控 + 令牌桶限流**：`GET /metrics` 暴露 11 个业务 + HTTP + 系统指标。`rate_limiter.py` 预置 strict/normal/import/ai 四档策略，超限返回 429 + Retry-After。
...
**合规审计**：`audit_logs` 表 JSON 格式全链路操作记录 + 88 次原子化 Git 提交 + AI 提示词版本化管理。
...
# gRPC AI 微服务（grpc_client.py 读取）
# GRPC_AI_HOST=localhost
# GRPC_AI_PORT=50051

# 主从库、Redis 等完整配置项见 .env.example
```
