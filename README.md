...
面向乡镇（街道）综治中心的一体化社会治理智能平台，践行新时代"枫桥经验"，以国产大模型与智能体技术赋能基层矛盾纠纷全周期管理。系统覆盖数据汇聚、智能去重、风险预警、处置跟进、分析报告、重点人员管理六大业务闭环，配套后台管理系统与完整 API 文档。
...
三级预警规则引擎自动扫描全量案件，跨渠道关联检测触发红/橙/黄三级预警，通过钉钉自动推送至相关责任人。

- **红色预警**：涉及金额 ≥10 万元或涉及死亡，立即推送综治中心负责人
- **橙色预警**：涉及金额 1 万 ~ 10 万元，2 小时内推送乡镇调解员
- **黄色预警**：难度级别非"简单纠纷"或跨 2 个渠道关联，当日提醒

预警事件独立记录，包含触发规则、关联渠道、推送状态，处置完成后自动归档。

**量化效果：高风险事件可提前 7 天主动发现，从被动响应转为主动预防。**
...
- **四后端 AI 插件架构**：云端 DeepSeek-v4-pro、本地 Sentence-Transformers 向量模型、本地 Ollama 大模型、规则引擎四套后端通过环境变量 `AI_BACKEND` 一键切换，统一 `semantic_similarity()` 接口零侵入。任一后端不可用时自动降级至下一级，确保服务永不断线。去重结果缓存 Redis（TTL 1h），避免重复调用 AI，兼顾准确率与成本控制。
...
- **审计追溯**：`audit_logs` 表以 JSON 格式记录每次操作的操作人、目标、详情和时间戳
